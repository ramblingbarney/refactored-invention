...
def convert_srt(raw_text):
    ''' Convert the song lycric stream to usable format '''
    ...
    for i in s:
        if cnt % 2 == 1:
            ...
            s2 = i.split('"')
            c = h.unescape(s2[4])
            #  Amended original code to replace &apos; with &#39;
            # as the former is not included on the list of HTML entities
            c2 = re.sub(r'&apos;', "&#39;", c[1:len(c)-2], re.IGNORECASE)
            ...
            times[cnt] = {
                "timeStart": int(tDelta1)*1000,
                "timeEnd": int(tDelta1 + tDelta2)*1000}
